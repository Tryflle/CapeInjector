package xyz.tryfle.capeinjector.util

/*
    GNU General Public License v3.0
    Copyright (C) 2023 Nilsen84
    https://github.com/Nilsen84/kt-bytecode-dsl
*/
...
abstract class InsnBuilder {
    ...
    fun lookupswitch(default: LabelNode, keys: IntArray, labels: Array<LabelNode>) =
    ...
    fun multianewarray(desc: String, dimensions: Int) =
        +MultiANewArrayInsnNode(desc, dimensions)
    ...
    fun f_new(local: Array<Any>, stack: Array<Any>) =
        +FrameNode(Opcodes.F_NEW, local.size, local, stack.size, stack)

    fun f_full(local: Array<Any>, stack: Array<Any>) =
        +FrameNode(Opcodes.F_FULL, local.size, local, stack.size, stack)

    fun f_append(vararg locals: Any) =
